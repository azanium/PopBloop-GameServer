...
namespace Photon.LoadBalancing.LoadShedding
{
    ...
    public class WorkloadController
    {
        ...
        private void StartTcpLatencyMonitor()
        {
            ...
            {
                if (this.application.ConnectToServer(
        ...
        private void Update()
        {
            ...
            if (this.tcpPeersCounter.InstanceExists && this.tcpDisconnectsPerSecondCounter.InstanceExists && this.tcpClientDisconnectsPerSecondCounter.InstanceExists)
            {
                try
                {
                    var tcpDisconnectsWithoutClientDisconnects = this.tcpDisconnectsPerSecondCounter.GetNextAverage() -
                                                                 this.tcpClientDisconnectsPerSecondCounter.GetNextAverage(); 

                    var tcpDisconnectRate = (int)(tcpDisconnectsWithoutClientDisconnects / this.tcpPeersCounter.GetNextAverage() * 1000);
            ...
            if (this.udpPeersCounter.InstanceExists && this.udpDisconnectsPerSecondCounter.InstanceExists && this.udpClientDisconnectsPerSecondCounter.InstanceExists)
            {
                try
                {
                    var udpDisconnectsWithoutClientDisconnects = this.udpDisconnectsPerSecondCounter.GetNextAverage() -
                                                                 this.udpClientDisconnectsPerSecondCounter.GetNextAverage();

                    var udpDisconnectRate = (int)(udpDisconnectsWithoutClientDisconnects / this.udpPeersCounter.GetNextAverage() * 1000);

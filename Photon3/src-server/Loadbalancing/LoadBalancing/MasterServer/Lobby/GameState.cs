...
namespace Photon.LoadBalancing.MasterServer.Lobby
{
    ...
    public class GameState
    {
        ...
        private bool UpdateProperties(Hashtable props)
        {
            ...
            if (!GameParameterReader.TryReadDefaultParameter(props, out maxPlayer, out isOpen, out isVisible, out propertyFilter, out debugMessage))
            {
                if (log.IsWarnEnabled)
                {
                    log.WarnFormat("Received invalid game properties: {0}", debugMessage);
                }
            ...
            {
                if (isVisible.Value != this.IsVisible)
                {
                    this.IsVisible = isVisible.Value;
                    changed = true;
                }
                
                props.Remove((byte)GameParameter.IsVisible);
            }

            if (isOpen.HasValue)
            {
                if (isOpen.Value != this.IsOpen)
                {
                    this.IsOpen = isOpen.Value;
                ...
                props.Remove((byte)GameParameter.IsOpen);
            }

            if (maxPlayer.HasValue)
            {
                if (maxPlayer.Value != this.MaxPlayer)
                {
                    this.MaxPlayer = maxPlayer.Value;
                    this.Properties[(byte)GameParameter.MaxPlayer] = maxPlayer.Value;
                    changed = true;
                }

                props.Remove((byte)GameParameter.MaxPlayer);
            }

            if (propertyFilter != null)
            {
                var lobbyProperties = new HashSet<object>(propertyFilter);
                ...
                props.Remove((byte)GameParameter.Properties);
            }

            foreach (object key in props.Keys)
            {
                object oldValue;
                object newValue = props[key];

                if (this.properties.TryGetValue(key, out oldValue))
                {
                    if (newValue == null)
                    {
                        changed = true;

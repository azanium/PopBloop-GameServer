...
namespace Photon.LoadBalancing.GameServer
{
    ...
    public class GameApplication : ApplicationBase
    {
        ...
        protected override void Setup()
        {
            ...
            IPAddress ipAddress;
            if (PublicIPAddressReader.TryParsePublicIpAddress(GameServerSettings.Default.PublicIPAddress, out ipAddress)
                == false)
            {
                log.ErrorFormat(
                    "Failed to get public ip address: settings={0}", GameServerSettings.Default.PublicIPAddress);
            }
